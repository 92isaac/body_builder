const imagePath = '/images'


export const heroImage = `${imagePath}/heroImage.png`
export const logo = `${imagePath}/logo.avif`
export const gymInstructor = `${imagePath}/gymGuy.png`
export const help1 = `${imagePath}/illustration1.png`
export const help2 = `${imagePath}/illustration2.png`
export const help3 = `${imagePath}/illustration3.png`
export const exercise1 = `${imagePath}/1.png`
export const exercise2 = `${imagePath}/2.png`
export const exercise3 = `${imagePath}/3.png`
export const exercise4 = `${imagePath}/4.png`
export const lotus = `${imagePath}/Lotus.png`
export const favicon = `${imagePath}/workout.png`
export const banner = `${imagePath}/banner.png`

